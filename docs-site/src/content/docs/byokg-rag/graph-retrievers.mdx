...
**`link(query_extracted_entities, retriever, topk, id_selector, return_dict, group_by_mention)`**

Links extracted entities to graph nodes using the configured retriever.

Parameters:
- `query_extracted_entities` (List[str]): Flat list of entity mention strings to link
- `retriever` (object, optional): Entity retriever to use for lookup
- `topk` (int, optional): Number of items to return per entity
- `id_selector` (list, optional): Allowlist of entity IDs to consider. Currently unused —
  kept in the signature so existing callers don't break; the `EntityMatcher` path routes
  through `index.match()`, which has no `id_selector` to forward it to.
- `return_dict` (bool): Whether to return detailed results or just entity IDs
- `group_by_mention` (bool): Return one candidate list per mention instead of a flat
  union. Overrides `return_dict`.

Returns:
- If `group_by_mention=True`: `List[List[str]]`, one best-first list of candidate node
  IDs per mention, in the same order as the mentions
- If `return_dict=True`: a single dict with a `hits` list — one hit per match, each
  field a scalar, globally sorted by score and therefore not sliceable per mention
- If `return_dict=False`: flat `List[str]` of matched entity IDs

`group_by_mention` is declared on the `Linker` base class, whose default implementation
groups by looking each mention up on its own — so a linker that implements only the flat
path still honours the flag rather than returning empty groups. It is what
`ByoKGQueryEngine(single_best_match=True)` uses to keep only each mention's best match —
without it the caller cannot tell which candidate came from which mention.

`link()` takes a flat `List[str]` of mentions (what `parse_response` returns). Nested
lists are rejected: the underlying index matches one string per input.

Grouping costs one retriever call per *unique* mention rather than one batched call.
The reason is `FuzzyStringIndex.match`, which concatenates every mention's hits and
re-sorts them globally by score, then drops candidates whose length differs too much
from the mention — so its flat result carries no per-mention boundaries and cannot be
sliced back apart. `LocalFaissDenseIndex.match` and `GraphStoreIndex.match` do return
hits grouped per input, but relying on that would make grouping index-specific.
Grouping changes only attribution, never which candidates survive: flattened, the
groups contain exactly the same candidates as the ungrouped union.
...
# Access linking results — one hit per match, each field a scalar
for hit in linking_results['hits']:
