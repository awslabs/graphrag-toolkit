...
`group_by_mention` is declared on the `Linker` base class, so any linker can be asked
to group. It is what `ByoKGQueryEngine(single_best_match=True)` uses to keep only each
mention's best match — without it the caller cannot tell which candidate came from
which mention.
