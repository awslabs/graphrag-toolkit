...
| `single_best_match` | bool | False | Keep only the best-matching node per extracted mention instead of unioning every top-k candidate. Reduces seed-set noise; draft answers are unaffected. Requires an entity linker providing `link_grouped`. | `True` |
