...
| `detect_source_id_collisions` | Whether a build fails when two different documents resolve to one source id. Costs one read per build batch | `True` | `DETECT_SOURCE_ID_COLLISIONS` |
