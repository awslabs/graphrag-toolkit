...
`entityId`, `value`, `search_str` and `class` are reserved by the graph model wherever a
typed property is keyed from the ontology's own vocabulary — that is, at
`typed_properties='subject'` and `'both'` — and `typed_value` and `datatype` are reserved in
addition at `'both'`. An ontology declaring a property with one of those names is refused at
configuration time. Nothing is reserved at `'off'` or at `'complement'` alone, because
neither keys a property from the ontology.
