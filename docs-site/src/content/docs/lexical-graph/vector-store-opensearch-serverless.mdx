...
### Topics
  ...
  - [Self-managed OpenSearch](#self-managed-opensearch)
...
You can also use a self-managed (non-AWS) OpenSearch instance — for example, a local Docker or Finch container — as a vector store. Supply a connection string that begins `opensearch://`, followed by the endpoint:
