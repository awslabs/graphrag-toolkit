...
As the extract stage stores a document, it writes a completion marker recording the chunks it wrote. A large document is stored in several parts, and the part that ends it records every chunk stored for that document. If the stage is interrupted, a document can be left with some of its chunks stored and nothing recording the rest.

The build stage skips a document whose markers do not account for the chunks stored alongside them, and logs a warning naming the S3 prefix. The document is not built, and the next extract run stores it again. Nothing else in the collection is affected.

A collection first written by a version that did not write markers is read as it stands, so documents already stored are not extracted a second time. Adding documents to such a collection leaves it that way; start a new `collection_id` to have the build stage check for incomplete documents.

#### Restarting an interrupted extraction

An extraction over a large corpus runs for hours or days, and a run that stops part way has paid for Bedrock batch jobs whose output is still in S3. Passing a `run_id` makes a run restartable: run it again with the same `run_id` and it takes up where the earlier run stopped rather than starting over.
...
graph_index.extract(
    docs,
    handler=run_plan_store.staging_handler('run-2026-09-24'),
    run_id='run-2026-09-24',
    run_plan_store=run_plan_store
)
```

The run records three things under `s3://<bucket>/<key_prefix>/<collection_id>/_runs/<run_id>/`, beside the collection it stages into:

- **A plan**, written once, before any work is submitted: the document ids in order and the batch size that divides them. A restart divides the input exactly as the first run did, even on a host with fewer cores, so it produces the same batch jobs. A restart whose `extraction_batch_size` or document set differs from the plan is refused with a `RunPlanMismatch`: that is a new run, not a restart of this one.
- **A record per partition** naming the Bedrock batch job it submitted. On a restart, a partition whose job completed has its output downloaded rather than submitted again; a job still running is waited on, since it bills to its end either way; a job that failed, expired, or described fewer nodes than the partition holds is submitted again as the next attempt. When the run finishes, the records are folded into one `rollup.json`.
- **Nothing new for staged documents.** `staging_handler` reads the collection and leaves alone every source document already stored whole, whichever run stored it, so the restart writes only what is missing. The JSONL format keeps chunk ids inside its objects where a listing cannot see them, so a JSONL handler stores everything again.

The plan does not yet cover the auto-tuned batch extraction path; a run with a `run_id` and `auto_tune=True` raises a `ValueError`.

The identity under which the lexical-graph runs needs `s3:PutObject`, `s3:GetObject` and `s3:DeleteObject` on `arn:aws:s3:::<bucket>/<key_prefix>/*`, and `s3:ListBucket` on `arn:aws:s3:::<bucket>`. `s3:DeleteObject` is used when a run with a `run_id` finishes and its partition records are folded into the rollup. Both stages list the objects under a collection: the extract stage to decide what is already stored, the build stage to read a document's chunks and its markers. Without `s3:ListBucket` these listings fail with an access denied error.
