...
As the extract stage stores a document, it writes a completion marker recording the chunks it wrote. If the stage is interrupted, a document can be left with some of its chunks stored and no marker.
